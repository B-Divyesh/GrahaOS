...
bool elf_load(void *file_data, uint64_t *entry_point, uint64_t *cr3) {
    klog(KLOG_DEBUG, SUBSYS_CORE, "[ELF] elf_load called");
    ...
    klog(KLOG_DEBUG, SUBSYS_CORE, "[ELF] Parameters OK, validating header...");
    ...
    klog(KLOG_DEBUG, SUBSYS_CORE, "[ELF] Header validated, drawing framebuffer msg...");
    framebuffer_draw_string("ELF: Header validated", 10, 420, COLOR_GREEN, 0x00101828);
    klog(KLOG_DEBUG, SUBSYS_CORE, "[ELF] Framebuffer msg drawn");

    // Create new address space
    klog(KLOG_DEBUG, SUBSYS_CORE, "[ELF] Creating address space...");
    ...
    klog(KLOG_DEBUG, SUBSYS_CORE, "[ELF] Address space created");
    ...
    // Get program headers
    klog(KLOG_DEBUG, SUBSYS_CORE, "[ELF] Getting program headers...");
    Elf64_Phdr *phdr = (Elf64_Phdr *)((uint8_t *)file_data + ehdr->e_phoff);
    klog(KLOG_DEBUG, SUBSYS_CORE, "[ELF] Program header count: %lu", (unsigned long)(ehdr->e_phnum));

    // Load each program segment
    for (int i = 0; i < ehdr->e_phnum; i++) {
        klog(KLOG_DEBUG, SUBSYS_CORE, "[ELF] Processing segment %lu type=0x%lx", (unsigned long)(i), (unsigned long)(phdr[i].p_type));

        if (phdr[i].p_type != PT_LOAD) {
            klog(KLOG_DEBUG, SUBSYS_CORE, "[ELF] Skipping non-LOAD segment");
            continue;
        }
        ...
        klog(KLOG_DEBUG, SUBSYS_CORE, "[ELF] Loading segment: vaddr=0x%lx memsz=0x%lx", (unsigned long)(vaddr), (unsigned long)(memsz));
        ...
        klog(KLOG_DEBUG, SUBSYS_CORE, "[ELF] Segment needs %lu pages", (unsigned long)(num_pages));

        klog(KLOG_DEBUG, SUBSYS_CORE, "[ELF] Drawing framebuffer msg...");
        framebuffer_draw_string("ELF: Loading segment", 10, 460 + (i * 20), COLOR_YELLOW, 0x00101828);
        klog(KLOG_DEBUG, SUBSYS_CORE, "[ELF] Framebuffer msg drawn");

        // Map and load pages
        for (size_t j = 0; j < num_pages; j++) {
            klog(KLOG_DEBUG, SUBSYS_CORE, "[ELF] Allocating page %lu/%lu...", (unsigned long)(j), (unsigned long)(num_pages));
            ...
            klog(KLOG_DEBUG, SUBSYS_CORE, "[ELF] Page allocated at 0x%lx", (unsigned long)((uint64_t)page_phys));

            // Get virtual address for copying
            void *page_virt = (void *)((uint64_t)page_phys + g_hhdm_offset);
            klog(KLOG_DEBUG, SUBSYS_CORE, "[ELF] Page virt addr: 0x%lx", (unsigned long)((uint64_t)page_virt));

            // Clear the page
            klog(KLOG_DEBUG, SUBSYS_CORE, "[ELF] Clearing page...");
            for (int k = 0; k < PAGE_SIZE; k++) {
                ((uint8_t *)page_virt)[k] = 0;
            }
            klog(KLOG_DEBUG, SUBSYS_CORE, "[ELF] Page cleared");

            // Copy segment data
            uint64_t page_vaddr = start_page + (j * PAGE_SIZE);
            klog(KLOG_DEBUG, SUBSYS_CORE, "[ELF] page_vaddr=0x%lx", (unsigned long)(page_vaddr));

            if (page_vaddr < vaddr + filesz) {
                klog(KLOG_DEBUG, SUBSYS_CORE, "[ELF] Copying segment data to page...");
                ...
                if (copy_size > 0) {
                    klog(KLOG_DEBUG, SUBSYS_CORE, "[ELF] Copying %lu bytes...", (unsigned long)(copy_size));
                    ...
                    klog(KLOG_DEBUG, SUBSYS_CORE, "[ELF] file_offset=0x%lx copy_start=0x%lx", (unsigned long)(file_offset), (unsigned long)(copy_start));

                    uint8_t *src = (uint8_t *)file_data + file_offset;
                    uint8_t *dst = (uint8_t *)page_virt + copy_start;

                    klog(KLOG_DEBUG, SUBSYS_CORE, "[ELF] src=0x%lx dst=0x%lx", (unsigned long)((uint64_t)src), (unsigned long)((uint64_t)dst));

                    klog(KLOG_DEBUG, SUBSYS_CORE, "[ELF] Starting memcpy loop...");
                    for (size_t k = 0; k < copy_size; k++) {
                        dst[k] = src[k];
                    }
                    klog(KLOG_DEBUG, SUBSYS_CORE, "[ELF] Copy complete");
                }
            }
            ...
            // Map the page
            klog(KLOG_DEBUG, SUBSYS_CORE, "[ELF] Mapping page at vaddr=0x%lx...", (unsigned long)(page_vaddr));
            ...
            klog(KLOG_DEBUG, SUBSYS_CORE, "[ELF] Page mapped successfully");
        }
    }

    klog(KLOG_DEBUG, SUBSYS_CORE, "[ELF] All pages loaded, drawing framebuffer msg...");
    framebuffer_draw_string("ELF: All segments loaded", 10, 540, COLOR_GREEN, 0x00101828);
    klog(KLOG_DEBUG, SUBSYS_CORE, "[ELF] Framebuffer msg drawn");

    klog(KLOG_DEBUG, SUBSYS_CORE, "[ELF] Setting output parameters...");
    *entry_point = ehdr->e_entry;
    *cr3 = vmm_get_pml4_phys(addr_space);
    klog(KLOG_DEBUG, SUBSYS_CORE, "[ELF] entry_point=0x%lx cr3=0x%lx", (unsigned long)(*entry_point), (unsigned long)(*cr3));

    klog(KLOG_DEBUG, SUBSYS_CORE, "[ELF] Building entry point message...");
    ...
    klog(KLOG_DEBUG, SUBSYS_CORE, "[ELF] Drawing entry point message...");
    framebuffer_draw_string(msg, 10, 560, COLOR_CYAN, 0x00101828);
    klog(KLOG_DEBUG, SUBSYS_CORE, "[ELF] Entry point message drawn");

    klog(KLOG_DEBUG, SUBSYS_CORE, "[ELF] elf_load returning true");
    return true;
}
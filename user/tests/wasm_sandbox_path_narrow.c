/* user/tests/wasm_sandbox_path_narrow.c — FU27.WASM.D2_worker gate test.
 *
 * Sends path_narrow_violator.wasm to wasmd.  The worker calls
 * gcp.fs_write on "evil.txt" which lacks the /tmp/ sandbox prefix,
 * traps with "cap denied: fs_write path outside sandbox", and exits
 * with code 5.  wasmd maps this to WASMD_E_CAP_DENIED (-10).
 *
 * Asserts:
 *   1. RUN_MODULE(path_narrow_violator) returns WASMD_E_CAP_DENIED.
 *   2. wasmd is still responsive (hello.wasm OK).
 *   3. hello's stdout still contains "hello" (no cross-talk).
 */
...
void _start(void) {
    ...
    if (wasmd_pid <= 0) {
        TAP_ASSERT(0, "wasm_sandbox_path_narrow: spawn bin/wasmd failed");
        TAP_ASSERT(0, "wasm_sandbox_path_narrow: skip");
        TAP_ASSERT(0, "wasm_sandbox_path_narrow: skip");
        syscall_exit(1);
    }

    static uint8_t v_buf[WASMD_BYTES_MAX];
    long v_n = read_fixture("bin/tests/wasm/path_narrow_violator.wasm",
                            v_buf, WASMD_BYTES_MAX);
    static uint8_t hello_buf[WASMD_BYTES_MAX];
    long hello_n = read_fixture("bin/tests/wasm/hello.wasm",
                                hello_buf, WASMD_BYTES_MAX);
    if (v_n <= 0 || hello_n <= 0) {
        TAP_ASSERT(0, "wasm_sandbox_path_narrow: read fixtures failed");
        TAP_ASSERT(0, "wasm_sandbox_path_narrow: skip");
        TAP_ASSERT(0, "wasm_sandbox_path_narrow: skip");
    ...
    int rc = libnet_connect_service_with_retry(WASMD_SERVICE_NAME,
                                               ...
                                               &cli);
    if (rc != 0) {
        TAP_ASSERT(0, "wasm_sandbox_path_narrow: connect /sys/wasm/control");
        TAP_ASSERT(0, "wasm_sandbox_path_narrow: skip");
        TAP_ASSERT(0, "wasm_sandbox_path_narrow: skip");
        if (wasmd_pid > 0) (void)syscall_kill(wasmd_pid, /*SIGKILL=*/2);
        syscall_exit(1);
    ...
    /* Phase 1: path_narrow_violator -> WASMD_E_CAP_DENIED. */
    int32_t  status1 = 0;
    uint8_t  out1[64];
    uint32_t out1len = 0;
    (void)run_module(&cli, v_buf, (uint32_t)v_n,
                     &status1, out1, sizeof(out1), &out1len);
    printf("# wasm_sandbox_path_narrow: status1=%d (expected %d)\n",
           status1, WASMD_E_CAP_DENIED);
    TAP_ASSERT(status1 == WASMD_E_CAP_DENIED,
               "wasm_sandbox_path_narrow: violator -> WASMD_E_CAP_DENIED");
    ...
    TAP_ASSERT(rc2 == 0 && status2 == WASMD_OK,
               "wasm_sandbox_path_narrow: hello OK after cap_denied (daemon survived)");

    int has_hello = contains_substring(out2, out2len, "hello");
    TAP_ASSERT(has_hello,
               "wasm_sandbox_path_narrow: hello stdout intact");
